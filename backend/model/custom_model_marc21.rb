class MARCModel < ASpaceExport::ExportModel
  ...
  def handle_primary_creator(linked_agents)
    ...
    if link['relator']
      ...
      role_info = ['4', relator]
   # no ma'am.  enough with the supplied data, ASpace!!!  how is anyone to know if the data was supplied vs. entered that way otherwise?  the current behavior goes against all rational logic.  granted, logic has its own downsides, but a lack of data isn't one of 'em.'
    else
      role_info = ''
    end
